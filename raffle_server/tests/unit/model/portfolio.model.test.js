...
describe('Portfolio Model test', () => {
  ...
  describe('Activity', () => {
    test('get Activity Basic : Address, Chain id only', async () => {
      const query = {
        chain_id: 1,
        address: _address,
      };
      await Portfolio.get_portfolio_activity(query);
    });

    test('get Activity Page and Limit option ', async () => {
      const _limit = 5;
      const query = {
        chain_id: 1,
        address: _address,
      ...
      const res = await Portfolio.get_portfolio_activity(query);
